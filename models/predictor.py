...
def scores_from_predictions(predictions: pd.Series,
    ...
    为什么不能直接用 generate_signals 的 score 列:那条路径的门控只作用在
    weight 上(select_top_k 里过滤),而策略读的是分数 —— 拿全截面 score 列
    就等于把"信号日没有成交就不许建仓"这道门整个绕开。
    ...
    if not isinstance(predictions.index, pd.MultiIndex):
        raise ValueError("scores_from_predictions 需要 (date, symbol) "
                         "MultiIndex 的预测序列")
    ...
    if tradable is not None and isinstance(tradable.index, pd.MultiIndex):
        ...
        # 掩码可以只含 True 项,也可以是全量 bool 序列 —— 两种都按"非 True 即不可选"
