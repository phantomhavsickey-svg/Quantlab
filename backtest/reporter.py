...
class ReportGenerator:
    ...
    @staticmethod
    def console_report(metrics: dict, factor_eval: dict | None = None) -> str:
        """生成格式化控制台文本报告。
        ...
        # 因子部分
        if factor_eval and factor_eval.get("ic_summary") is not None:
            lines.append("")
            lines.append("[Factor Evaluation - Top 5 ICIR]")
            lines.append("-" * 70)
            top5 = factor_eval["ic_summary"].head(5)
            for fname, row in top5.iterrows():
                lines.append(f"  {fname:30s} ICIR={row['ICIR']:7.3f}  "
                             f"IC_mean={row['IC_mean']:6.4f}  "
                             f"IC>0={row['IC_positive_ratio']:.0%}")
    ...
    def html_report(self, equity_curve: pd.Series,
        ...
        """
        if not HAS_PLOTLY:
            logger.warning("plotly 未安装，无法生成 HTML 报告")
            return ""
        ...
        # 1. 净值曲线
        fig_nv = self.plot_equity_curve(equity_curve, benchmark_curve)
        figs.append(fig_nv)

        # 2. 回撤曲线
        fig_dd = self.plot_drawdown(equity_curve)
        figs.append(fig_dd)

        # 3. 月度收益热力图
        if daily_returns is not None:
            fig_monthly = self.plot_monthly_heatmap(
                self._returns_to_equity(daily_returns))
            figs.append(fig_monthly)

        # 拼接 HTML
        html_parts = ["<html><head><meta charset='utf-8'>"
                      "<title>QuantLab 回测报告</title>"
    ...
    def plot_drawdown(equity_curve: pd.Series) -> "go.Figure":
        ...
        drawdown = (equity_curve - peak) / peak * 100  # 百分比
    ...
    @staticmethod
    def plot_ic_cumulative(ic_df: pd.DataFrame) -> "go.Figure":
        """绘制累计 IC 曲线。"""
        cum_ic = ic_df.cumsum()

        fig = go.Figure()
        for col in cum_ic.columns[:10]:  # 最多画10个因子
            fig.add_trace(go.Scatter(
                x=cum_ic.index,
                y=cum_ic[col].values,
                mode="lines",
